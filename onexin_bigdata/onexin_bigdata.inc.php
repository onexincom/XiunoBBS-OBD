<?php
/**
 * ONEXIN BIG DATA For phpmps 5.5+
 * ============================================================================
...
//		if(isset($_POST['isdelimiter'])) $options['isdelimiter'] = (int)$_POST['isdelimiter'];
	...
	if(bigdata_submitcheck('loginsubmit')) {
        if( md5(md5($_POST['username']) . md5($_POST['password'])) == $md5 ){
            $_SESSION['obd'] = 1;
        }  
        echo '<script>location.href=\'?op=stats\'</script>';
        exit;
    }else{
	    include bigdata_template('onexin_bigdata:login');
        exit;
    }
