...
$catid = explode('|', $_POST['catid']);
        ...
        $contents       = explode('[hr]', $_POST['content']);  
    
		$time           = time();
        $title          = addslashes(htmlspecialchars($_POST['title']));
		$content        = addslashes(trim($contents[0]));
		$uid            = $userid;
		$fid            = intval($catid[0]);
		$create_date    = $time;
		$last_date      = $time;
        ...
        if($contents) {
            ...
            foreach ($contents as $key => $value) {                
                $addtime   = time() + $key * rand(5, 20) + rand(5, 100);
                $uid       = intval($userid);//回复者uid
                $content   = addslashes(trim($value));
                $inserts[] = "('$art_id','$uid','0','$addtime','$content','$content')";
            }
            DB::query("INSERT INTO ".DB::table('post')." (tid,uid,isfirst,create_time,message,message_fmt) VALUES ".implode(',', $inserts));
        } 
